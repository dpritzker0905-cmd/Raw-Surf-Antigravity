...
MARINE_COMPONENT_LAYERS = ("waves", "swell_1", "swell_2", "wind_waves")


async def resolve_conjoined_marine_grid(
    store,
    viewport_service,
    *,
    model: str,
    ...
    """
    Resolve ALL four marine component layers (waves, swell_1, swell_2, wind_waves) and merge them
    into ONE "conjoined" grid: each vector carries {waves, swell_1, swell_2, wind_waves} sub-objects
    plus a top-level (waves) for non-conjoined-aware consumers.

    This lets the client fetch every marine layer in a SINGLE request and toggle between layers
    purely client-side (no refetch, no blank). It is strictly additive — it reuses resolve_grid per
    component (so all cache/viewport/coverage/fallback behavior is identical), and the existing
    per-layer /grid path is untouched. The upstream provider already fetches an `all_marine` grid in
    one call, so resolving waves first warms the cache for the other 3 components → the parallel
    batch below mostly hits cache instead of firing 4 concurrent upstream fetches on the 1-CPU box.
    """
    async def _resolve_one(lyr):
        try:
            res = await resolve_grid(
                store, viewport_service,
                model=model, domain="marine", layer=lyr, valid_time=valid_time,
                bbox=bbox, background_tasks=background_tasks, request=request,
            )
            return lyr, res
        except HTTPException:
            # Propagate disconnect (499) / real HTTP errors so the handler can bail cleanly.
            raise
        except Exception as e:
            logger.warning(f"[Conjoined] marine component '{lyr}' resolution failed: {e}")
            return lyr, None

    # waves first (warms the shared all_marine cache), then the rest in parallel (cache hits).
    _, waves_res = await _resolve_one("waves")
    others = await asyncio.gather(*[_resolve_one(l) for l in ("swell_1", "swell_2", "wind_waves")])

    by_layer = {}
    for lyr, res in [("waves", waves_res)] + list(others):
        if isinstance(res, NormalizedProduct) and res.grid and res.grid.vectors:
            by_layer[lyr] = res

    spine = by_layer.get("waves") or next(iter(by_layer.values()), None)
    if spine is None:
        # No component resolved to a renderable grid → honest empty/no-coverage response.
        return make_no_coverage_grid_response(model, "all_marine", valid_time)

    def _key(v):
        return (round(v.lat, 4), round(v.lng, 4))

    layer_maps = {lyr: {_key(v): v for v in prod.grid.vectors} for lyr, prod in by_layer.items()}

    def _sub(v):
        if v is None:
            return {"u": 0.0, "v": 0.0, "speed": 0.0, "period": 0.0, "height": 0.0, "direction": 0.0, "isOcean": False}
        return {
            "u": v.u, "v": v.v, "speed": v.speed,
            "period": v.period if v.period is not None else 0.0,
            "height": v.speed,
            "direction": v.direction,
            "isOcean": bool(v.is_valid),
        }

    conjoined = []
    for sv in spine.grid.vectors:
        k = _key(sv)
        cv = {
            "lat": sv.lat, "lng": sv.lng,
            "speed": sv.speed, "direction": sv.direction, "u": sv.u, "v": sv.v,
            "period": sv.period, "is_valid": sv.is_valid,
        }
        for lyr in MARINE_COMPONENT_LAYERS:
            m = layer_maps.get(lyr)
            cv[lyr] = _sub(m.get(k)) if m else _sub(None)
        conjoined.append(cv)

    g = spine.grid
    renderable = any(
        cv["waves"]["speed"] > 0 or cv["swell_1"]["speed"] > 0 or
        cv["swell_2"]["speed"] > 0 or cv["wind_waves"]["speed"] > 0
        for cv in conjoined
    )
    return JSONResponse(status_code=200, content={
        "model": spine.model,
        ...
        "domain": "marine",
        "layer": "all_marine",
        "run_time": spine.run_time.isoformat() if getattr(spine, "run_time", None) else datetime.now(timezone.utc).isoformat(),
        "valid_time": spine.valid_time.isoformat() if getattr(spine, "valid_time", None) else parse_valid_time(valid_time).isoformat(),
        "is_forecast_authoritative": getattr(spine, "is_forecast_authoritative", True),
        "is_estimated": getattr(spine, "is_estimated", False),
        "product_id": getattr(spine, "product_id", None),
        "coverage_scope": getattr(spine, "coverage_scope", None),
        "coverage_mode": getattr(spine, "coverage_mode", None),
        "is_dynamic_viewport_product": getattr(spine, "is_dynamic_viewport_product", False),
        "requested_bbox": getattr(spine, "requested_bbox", bbox),
        "served_bbox": getattr(spine, "served_bbox", None),
        "stale": getattr(spine, "stale", False),
        "value_kind": "wave_height",
        "value_unit": "m",
        "display_unit_hint": "ft",
        "units": {"speed": "m", "direction": "degrees", "period": "seconds"},
        "conjoined": True,
        "components": list(by_layer.keys()),
        "grid": {
            ...
            "diagnostics": {
                "conjoined": True,
                "components": list(by_layer.keys()),
                "vectors_length": len(conjoined),
                "nonzeroCount": sum(1 for cv in conjoined if cv["waves"]["speed"] > 0),
