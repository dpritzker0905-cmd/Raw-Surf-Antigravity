...
from services.weather_pipeline.route_helpers import (
    parse_bbox, clamp_and_normalize_bbox,
...
# Upstream fetch + normalize/persist blocks split to viewport_upstream.py (2026-07-10, LOC gate);
# the shared in-flight context (locks/shields/futures — db94a7c3) stays in this file.
...
class ViewportService:
    ...
    async def fetch_viewport_grid_upstream(
        ...
        try:
            raw_data, resolution, coord_count, bbox_dict = await fetch_upstream_raw(
                service=self, model=model, domain=domain, layer=layer,
                valid_time_str=valid_time_str,
                west=west, south=south, east=east, north=north,
                resolution=resolution, is_global_view=is_global_view,
            ...
            target_normalized_product = await normalize_and_persist_layers(
                service=self, model=model, domain=domain, layer=layer,
                ...
                coverage_scope=coverage_scope, coord_count=coord_count
            )
