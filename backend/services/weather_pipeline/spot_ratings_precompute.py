"""spot_ratings_precompute.py — the PRECOMPUTE + L2 PERSISTENCE lane for spot ratings.

SPLIT OUT OF `spot_ratings.py` 2026-08-14 (Program 13.0 Mission 2). That file had reached the hard
800-LOC ceiling with THREE lines of headroom, so the next edit to the repo's mandated rating
reference implementation was blocked on a refactor — which is the worst moment to be forced into one.

⭐ THE SEAM WAS MEASURED, NOT GUESSED. An AST census of the original file found exactly TWO
cross-half edges between the rating compute and this lane:

    rate_one_spot()            -> _iso_z()          (a 15-line datetime formatter; it stayed behind,
                                                     because NOTHING here uses it — verified)
    precompute_spot_ratings()  -> rate_one_spot()   (the correct direction, and the only one kept)

So the dependency runs ONE WAY: this module imports the rating, the rating never imports this.
⛔ DO NOT ADD AN IMPORT FROM `spot_ratings` TO THIS MODULE. That would create the cycle the split
exists to prevent, and it is the only way this seam can rot.

★ WHAT DID **NOT** CHANGE, and must not: `spot_ratings.rate_one_spot` is still the ONE reference
implementation named by CLAUDE.md's ONE FORECAST COMPOSITION mandate, still at its original path.
This module does not re-derive any of it — `precompute_spot_ratings` calls it. Ten test files read
`spot_ratings.py` BY PATH as a source string (glyph disclosure, flag-lane parity, the three-surface
behavioural census); the split direction was chosen so those keep grading the same file. The two
that grade content which moved HERE — `test_climatology_inbox` (RATING_SIZE_CLIMATOLOGY) and
`test_observation_gate_single_model_surfaces` (RATING_OBS_GATE) — were repointed in the same commit,
and this module was added to `test_flag_lane_parity._RATING_SURFACES` so its four science switches
(RATING_LOCAL_SIZE, RATING_TIDE, RATING_OBS_GATE, RATING_SIZE_CLIMATOLOGY) stay visible to the
admin panel and to every lane guard. Moving a flag out of the scanned set is how a switch goes dark.

⚠️ `SPOT_RATINGS_L2_KEY` / `SPOT_RATINGS_SCHEMA_VERSION` deliberately stayed in `spot_ratings.py`:
that key is pinned by a Supabase storage RLS policy AND by the frontend CDN client, and its warning
...
# ⛔ ONE DIRECTION ONLY — see the header. The rating is the dependency; this lane is the dependent.
from services.weather_pipeline.spot_ratings import (
    SPOT_RATINGS_L2_KEY,
    SPOT_RATINGS_SCHEMA_VERSION,
    rate_one_spot,
)
