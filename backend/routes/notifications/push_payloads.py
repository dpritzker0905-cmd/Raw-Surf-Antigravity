"""
push_payloads.py — Domain-specific push notification helper functions.

Extracted from push.py (v88 decomposition). Contains notification trigger
functions for: live sessions, groms, crew hub, dispatch, gallery, social,
surf alerts, bookings, and reviews.

All functions import `send_push_notification` from the parent push module.
"""
...
# ============ RE-EXPORT send_push_notification for internal callers ============
...
async def notify_session_join(photographer_id: str, surfer_name: str, amount: float, spot_name: str, db: AsyncSession = None):
    """Push notification to photographer when surfer joins their live session."""
...
async def notify_grom_activity(parent_id: str, grom_name: str, activity_type: str, detail: str = "", db: AsyncSession = None):
    ...
    try:
        titles = {
            'post': f"{grom_name} posted!", 'live': f"{grom_name} went live!",
            'session': f"{grom_name} logged a surf session", 'achievement': f"{grom_name} earned a badge!"
        }
        messages = {
...
async def notify_grom_link_request(parent_id: str, grom_name: str, grom_email: str, db: AsyncSession = None):
    ...
    try:
        return await send_push_notification(
            user_id=parent_id, title="Grom Link Request",
...
async def notify_crew_payment_request(crew_member_id: str, captain_name: str, captain_avatar: str, booking_id: str, location: str, share_amount: float, expires_at: str, booking_type: str = "scheduled", db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            ...
            data={"type": "crew_payment_request", "booking_id": booking_id, "captain_name": captain_name,
                  "captain_avatar": captain_avatar, "location": location, "share_amount": share_amount,
                  "expires_at": expires_at, "booking_type": booking_type, "deep_link": f"/bookings/pay/{booking_id}"},
...
async def notify_crew_payment_received(captain_id: str, crew_member_name: str, amount: float, booking_id: str, remaining_balance: float, db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            user_id=captain_id, title="Crew Payment Received!", message=message,
            data={"type": "crew_payment_received", "booking_id": booking_id, "crew_member_name": crew_member_name,
                  "amount": amount, "remaining_balance": remaining_balance, "deep_link": f"/bookings/pay/{booking_id}"},
            action_url=f"/bookings/pay/{booking_id}"
        )
    finally:
        if close_session:
...
async def notify_crew_payment_expiring(crew_member_id: str, captain_name: str, booking_id: str, share_amount: float, minutes_remaining: int, db: AsyncSession = None):
    ...
    try:
        return await send_push_notification(
            user_id=crew_member_id, title="Payment Reminder!",
            message=f"Only {minutes_remaining} min left to pay ${share_amount:.2f} for {captain_name}'s session!",
            data={"type": "crew_payment_expiring", "booking_id": booking_id, "share_amount": share_amount,
                  "minutes_remaining": minutes_remaining, "deep_link": f"/bookings/pay/{booking_id}"},
            action_url=f"/bookings/pay/{booking_id}"
        )
    finally:
        if close_session:
...
async def notify_crew_session_confirmed(participant_id: str, captain_name: str, booking_id: str, location: str, session_date: str, db: AsyncSession = None):
    ...
    try:
        return await send_push_notification(
            user_id=participant_id, title="Session Confirmed!",
            message=f"All set! {captain_name}'s session at {location} is confirmed. See you there!",
            data={"type": "crew_session_confirmed", "booking_id": booking_id, "captain_name": captain_name,
...
async def notify_photographer_arrived(surfer_id: str, photographer_name: str, dispatch_id: str, db: AsyncSession = None):
    ...
    try:
        if not await should_send_notification(surfer_id, 'dispatch', db):
            return {"status": "skipped", "reason": "user_preference"}
        return await send_push_notification(
            user_id=surfer_id, title=f"{photographer_name} has arrived! 📸",
            message="Your photographer is at the meeting point. Time to shred!",
            data={"type": "photographer_arrived", "dispatch_id": dispatch_id, "photographer_name": photographer_name},
...
async def notify_session_completed(surfer_id: str, photographer_name: str, dispatch_id: str, duration_mins: int, gallery_id: str = None, db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            user_id=surfer_id, title="Session Complete! 🤙",
            message=f"{photographer_name} finished your {dur_text}. Your gallery is being prepared!",
            data={"type": "session_completed", "dispatch_id": dispatch_id, "gallery_id": gallery_id, "duration_mins": duration_mins},
...
async def notify_photos_ready(surfer_id: str, photographer_name: str, gallery_id: str, photo_count: int = 0, db: AsyncSession = None):
    """Push notification when gallery photos are uploaded and ready."""
    from routes.notifications.notification_preferences import should_send_notification
    from database import AsyncSessionLocal
    close_session = False
    if db is None:
    ...
    try:
        ...
        return await send_push_notification(
            user_id=surfer_id, title="Your Photos Are Ready! 📸",
            message=f"{photographer_name} uploaded {count_text}. Check them out!",
            data={"type": "photos_ready", "gallery_id": gallery_id, "photo_count": photo_count},
...
async def notify_photos_found_ai(surfer_id: str, gallery_id: str, match_count: int, spot_name: str = "", db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            user_id=surfer_id, title=f"We found {match_count} photos of you! 🤖📸",
            message=f"AI matched {match_count} photos{location_text}. Tap to view!",
            data={"type": "photos_found_ai", "gallery_id": gallery_id, "match_count": match_count},
            action_url=f"/gallery/{gallery_id}"
        )
    finally:
        if close_session:
...
async def notify_new_comment(author_id: str, commenter_name: str, post_id: str, preview: str = "", db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            user_id=author_id, title=f"{commenter_name} commented",
            message=f"{commenter_name} commented {msg}",
            data={"type": "new_comment", "post_id": post_id, "commenter_name": commenter_name},
            action_url=f"/post/{post_id}", urgent=False
        )
    finally:
        if close_session:
            await db.close()
...
async def notify_mention(mentioned_user_id: str, mentioner_name: str, post_id: str, db: AsyncSession = None):
    ...
    try:
        if not await should_send_notification(mentioned_user_id, 'social', db):
            return {"status": "skipped", "reason": "user_preference"}
        return await send_push_notification(
            user_id=mentioned_user_id, title=f"{mentioner_name} mentioned you",
            message=f"{mentioner_name} mentioned you in a post. Tap to see!",
            data={"type": "mention", "post_id": post_id, "mentioner_name": mentioner_name},
...
async def notify_surf_alert_triggered(user_id: str, spot_name: str, spot_id: str, alert_summary: str, db: AsyncSession = None):
    ...
    try:
        if not await should_send_notification(user_id, 'alerts', db):
            return {"status": "skipped", "reason": "user_preference"}
        return await send_push_notification(
            user_id=user_id, title=f"🌊 Surf Alert: {spot_name}", message=alert_summary,
            data={"type": "surf_alert", "spot_id": spot_id, "spot_name": spot_name},
...
async def notify_booking_reminder(user_id: str, photographer_name: str, booking_id: str, location: str, time_until: str, db: AsyncSession = None):
    """Push notification for upcoming booking reminder."""
    from routes.notifications.notification_preferences import should_send_notification
    from database import AsyncSessionLocal
    close_session = False
    if db is None:
    ...
    try:
        if not await should_send_notification(user_id, 'bookings', db):
            return {"status": "skipped", "reason": "user_preference"}
        return await send_push_notification(
            user_id=user_id, title=f"Session in {time_until}! 🏄",
            message=f"Your session with {photographer_name} at {location} starts in {time_until}.",
            data={"type": "booking_reminder", "booking_id": booking_id, "time_until": time_until},
            action_url="/bookings"
        )
    finally:
        if close_session:
...
async def notify_gallery_purchase(photographer_id: str, buyer_name: str, gallery_id: str, item_count: int, amount: float, db: AsyncSession = None):
    ...
    try:
        ...
        return await send_push_notification(
            user_id=photographer_id, title=f"New Sale! 💰 ${amount:.2f}",
            message=f"{buyer_name} purchased {items_text} from your gallery!",
            data={"type": "gallery_purchase", "gallery_id": gallery_id, "buyer_name": buyer_name, "amount": amount},
            action_url=f"/gallery/{gallery_id}", urgent=False
        )
    finally:
        if close_session:
            await db.close()
...
async def notify_review_received(photographer_id: str, reviewer_name: str, rating: float, db: AsyncSession = None):
    ...
    try:
        ...
        stars = "⭐" * int(rating)
        return await send_push_notification(
            user_id=photographer_id, title=f"New Review! {stars}",
            message=f"{reviewer_name} left you a {rating:.1f}-star review. Nice work!",
            data={"type": "review_received", "reviewer_name": reviewer_name, "rating": rating},
            action_url="/profile", urgent=False
        )
    finally:
        if close_session:
            await db.close()
