...
def _baseline_for(spot: Dict[str, Any]) -> Optional[Dict[str, float]]:
    ...
    Precedence: a staged simulation override, then the hand-tuned catalog defaults. Database spots
    have neither — the catalog stores no forecast and this server does no network I/O — so they
    return None rather than an invented number."""
...
def get_weather_forecast(spot_name: str) -> Dict[str, Any]:
    ...
    if baseline is None:
        ...
        payload["note"] = ("No baseline forecast is stored for this spot — this server holds no "
                           "forecast data and performs no network I/O. Use "
                           "simulate_weather_change() to evaluate a specific weather vector.")
        return payload

    payload["forecast"] = baseline
    payload["baseline_source"] = ("simulated_override" if spot.get("name") in _SIM_OVERRIDES
