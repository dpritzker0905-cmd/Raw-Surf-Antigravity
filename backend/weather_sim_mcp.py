...
import urllib.error
import urllib.parse
# `urllib.request` pulls `ssl`, whose `_ssl` C extension must be loaded on the MAIN thread — the
# same class of lazy C-extension import that deadlocked this server's first tool call (see the
# warm-up note at the bottom of this file). Imported HERE, never inside a tool.
...
# ── THE APP'S OWN FORECAST, for every spot in the catalog ────────────────────────────────────
# Until 2026-07-27 `get_weather_forecast` could only answer for the THREE hand-tuned spots below:
# every one of the other 1544 catalog spots returned `forecast: null`, because this server stored no
# forecast and did no network I/O. A forecast tool that cannot forecast 99.8% of the catalog is not
# a sandbox limitation, it is the missing feature.
#
# The app already serves exactly this at `/api/weather/point`, off the same cached product grid the
# heatmaps sample, so the sim ASKS IT rather than modelling a second forecast. The mapping is copied
# from `spot_ratings.rate_one_spot` — the production consumer of the same endpoint — so the sim
# cannot drift from it:
#     swell height  <- marine point.speed      (OFFSHORE Hs; the sim's own chain breaks it)
#     period        <- marine point.period
#     swell dir     <- marine point.direction
#     wind knots    <- wind point.speed        (that endpoint reports wind in knots)
#     wind dir      <- wind point.direction
# The response also carries `surf_height_m`, the BREAKING height the app serves. It is kept as a
# parity check: the sim computes its own from the offshore Hs, and if the two ever diverge the
# forecast payload shows it instead of hiding it. Measured 2026-07-27 at Mavericks: served 2.3077 m
# vs sim 2.3165 m = +0.38%, which is the 0.1 ft rounding of the reported value and nothing else.
#
# Kill: SIM_LIVE_FORECAST=0 restores the pre-07-27 "catalog defaults only" behaviour exactly.
FORECAST_BASE_URL = os.environ.get(
    "SIM_FORECAST_BASE_URL", "https://raw-surf-antigravity.onrender.com").rstrip("/")
FORECAST_MODEL = os.environ.get("SIM_FORECAST_MODEL", "GFS")
FORECAST_TIMEOUT_S = float(os.environ.get("SIM_FORECAST_TIMEOUT_S", "30"))
...
def _fetch_point(domain: str, layer: str, lat: float, lng: float,
                 valid_time: str) -> Optional[Dict[str, Any]]:
    """One `/api/weather/point` sample, or None. Never raises — a forecast is an ENRICHMENT."""
    qs = urllib.parse.urlencode({
        "model": FORECAST_MODEL, "domain": domain, "layer": layer,
        "lat": lat, "lng": lng, "valid_time": valid_time})
    url = f"{FORECAST_BASE_URL}/api/weather/point?{qs}"
    try:
        with urllib.request.urlopen(url, timeout=FORECAST_TIMEOUT_S) as resp:
...
def fetch_live_forecast(lat: float, lng: float
    ...
    Both legs are required: with only one of them the vector would have to invent the other, and an
    invented wind is exactly how a blown-out day reads clean."""
    if not _sim_flag("SIM_LIVE_FORECAST"):
    ...
    marine = _fetch_point("marine", "waves", lat, lng, valid_time)
    wind = _fetch_point("wind", "wind", lat, lng, valid_time)
    ...
    if missing:
        out = (None, {"reason": f"no {' and '.join(missing)} data at this coordinate",
                      "valid_time": valid_time, "model": FORECAST_MODEL})
    else:
        baseline = {
...
def _baseline_with_source(spot: Dict[str, Any]
    ...
    if lat is not None and lng is not None:
        live, provenance = fetch_live_forecast(float(lat), float(lng))
        if live is not None:
