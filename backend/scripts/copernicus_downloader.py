...
def download_and_parse():
    ...
    # ── CASE 1: Subcommand Mode ──
    if cmd in ("download", "parse"):
        if cmd == "download":
            if len(sys.argv) < 4:
                print("Error: Missing temp NetCDF file path argument.")
                sys.exit(1)
            input_path = Path(sys.argv[2])
            temp_nc_path = Path(sys.argv[3])
            ...
            os.environ["COPERNICUSMARINE_CREDENTIALS_DIRECTORY"] = str(temp_nc_path.parent)
            
            import copernicusmarine
            if temp_nc_path.exists():
                temp_nc_path.unlink()
                
            print(f"Downloading subset to {temp_nc_path}...")
            try:
                copernicusmarine.subset(
                    ...
                    output_directory=str(temp_nc_path.parent),
            ...
            if len(sys.argv) < 5:
                print("Error: Missing output JSON file path for parse command.")
                sys.exit(1)
            input_path = Path(sys.argv[2])
            temp_nc_path = Path(sys.argv[3])
            ...
            print(f"Parsing downloaded NetCDF file {temp_nc_path} using netCDF4...")
            try:
                nc = netCDF4.Dataset(temp_nc_path, "r")
    ...
    # ── CASE 2: Legacy Mode (Download + Parse in one process) ──
    else:
