...
# ── THE INTERRUPT SHIELD: this job shares a console with whatever else uses the machine ─────────
# ATTRIBUTED 2026-08-04 after SIX deaths in one day, every one exiting `0xC000013A`
# (STATUS_CONTROL_C_EXIT) with `^C` in the log. The scheduled task runs `LogonType: Interactive`,
# so it lands in the user's INTERACTIVE SESSION — measured: campaign sessionId 1, agent shell
# sessionId 1, the same one. A CTRL_C generated by ANY long-running command in that session is
# broadcast to the console process group and takes the campaign with it. Five of the six deaths
# coincided with a long foreground command; a short one was controlled for and never did it.
#
# ⛔ THE REAL FIX IS THE SESSION, NOT THE SIGNAL, and it needs elevation this process does not have:
#   Set-ScheduledTask -TaskName '<task>' -Principal (New-ScheduledTaskPrincipal `
#       -UserId $env:USERNAME -LogonType S4U -RunLevel Limited)
# S4U runs the task outside the interactive session entirely. Until that is applied, this shield
# stops the SYMPTOM: a batch job that is not attached to a human has no business dying on someone
# else's Ctrl-C.
#
# ★ AUTO-DETECT RATHER THAN A FLAG, so the scheduled task needs no argument change (which also
#   requires elevation here): under the task, stdout is redirected to the log and is NOT a tty; in a
#   terminal it is, and a human keeps Ctrl-C. `ERA5_INTERRUPT_SHIELD=1|0` forces either way.
def _shield_wanted():
    mode = os.environ.get("ERA5_INTERRUPT_SHIELD", "auto").strip().lower()
    if mode in ("1", "true", "yes", "on"):
        return True
    if mode in ("0", "false", "no", "off"):
        return False
    try:
        return not sys.stdout.isatty()          # redirected => nobody is watching to press Ctrl-C
    except Exception:                           # noqa: BLE001 - a detached stdout can raise
        return True


def install_interrupt_shield(force=None):
    """Ignore SIGINT when this run is not attached to a terminal. Returns True if the shield went on.

    ⚠️ Deliberately does NOT shield SIGTERM or the Windows CTRL_CLOSE/LOGOFF/SHUTDOWN events: a
    deliberate `Stop-ScheduledTask`, a reap by `_reap_if_wedged`, or a real shutdown must still stop
    this job. The shield is aimed at ONE thing — an unrelated console interrupt in a shared session.
    """
    want = _shield_wanted() if force is None else bool(force)
    if not want:
        return False
    try:
        signal.signal(signal.SIGINT, signal.SIG_IGN)
        return True
    except Exception as e:                       # noqa: BLE001 - never let telemetry kill the job
        print(f"  (interrupt shield could not be installed: {e!r})", flush=True)
        return False
