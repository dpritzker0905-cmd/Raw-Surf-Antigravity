...
def test_all_non_gfs_layers(mock_weather_setup):
    ...
    for c in cases:
        ...
        # provider/upstream_provider honesty (always open-meteo for viewport grid, except EURO waves/swell/wind_waves fallbacks)
        expected_provider = "gfs_estimated_fallback" if (model == "EURO" and layer in ("waves", "swell_1", "swell_2", "wind_waves")) else "open-meteo"
...
async def test_viewport_background_processing_and_no_supabase_upload(mock_weather_setup, monkeypatch):
    ...
    # We will mock fetch_grid to return multiple hours
...
async def test_viewport_concurrent_waiters_prioritization(mock_weather_setup, monkeypatch):
    ...
    # Wait a tiny bit, then send Request B for 14:00 (which is different from 12:00 but uses the same bbox/fetch)
