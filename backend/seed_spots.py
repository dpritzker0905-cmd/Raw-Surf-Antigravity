...
async def seed():
    ...
    async with async_session_maker() as db:
        print('Connected! Checking for spots...')
        success = 0
        for s in missingSpots:
            import datetime
            # Check if exists
            result = await db.execute(text("SELECT COUNT(*) FROM surf_spots WHERE name = :name"), {'name': s['name']})
            count = result.scalar()
            if count == 0:
                spot = SurfSpot(
                    name=s['name'],
                ...
                success += 1
        await db.commit()
        print(f'Successfully injected {success} spots!')
