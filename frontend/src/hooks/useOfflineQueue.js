/**
 * useOfflineQueue G현 Queues user actions when offline, auto-syncs on reconnect.
 *
...
const saveQueue = (queue) => {
  try {
    ...
    // localStorage full G현 drop oldest items
...
const replayAction = async (action) => {
  ...
  try {
    ...
    if (err.response && err.response.status >= 400 && err.response.status < 500) {
      console.warn(`[OfflineQueue] Dropping action (${type}) G현 ${err.response.status}`);
      return true;
    }
...
export const useOfflineQueue = () => {
  ...
  }, [isOnline]); // flushQueue intentionally omitted G현 guarded by syncingRef
  ...
  const queueAction = useCallback((type, payload) => {
    ...
    toast.info('=껚� Saved offline G현 will sync when back online');
  }, []);
