 ...
 * Extracted from SurferGallery.js G현 handler logic for the Locker page.
 * 
 * Extraction checklist (v30):
 * G｀ Step 1: JSX scan G현 zero JSX in extraction range (lines 716-921)
 * G｀ Step 2: Closure param audit G현 all state/setters listed
 * G｀ Step 3: Phantom param check G현 every param used in handler body
 * G｀ Step 4: React hooks scan G현 no React hooks needed (pure handlers)
 * G｀ Step 5: Utility/import scan G현 apiClient, toast, logger, getFullUrl
 * G｀ Step 6: Hook placement G현 called AFTER all useState declarations
 * G｀ Step 7: Cross-module export G현 no constants needed
 */
import apiClient from '../lib/apiClient';
import { toast } from 'sonner';
import logger from '../utils/logger';

const useSurferGalleryActions = ({
  user,
  galleryItems,
  selectedItems,
  multiSelectMode,
