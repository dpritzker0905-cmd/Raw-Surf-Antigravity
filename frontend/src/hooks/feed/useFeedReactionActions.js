/**
 * useFeedReactionActions G현 Extracted from useFeedActions.js (v77)
...
const useFeedReactionActions = ({
  ...
  const handleLike = async (postId) => {
    ...
    // Optimistic UI update G현 also clear user_reaction on unlike
  ...
  const handleShakaPointerDown = (postId, e) => {
    ...
    // Capture anchor rect eagerly G현 React recycles the synthetic event,
  ...
  const handleShakaClick = (postId) => {
    // Only act if pointerUp didn't already handle it G현 check elapsed time.
  ...
  const handleReaction = async (postId, emoji) => {
    ...
    // Optimistic update G현 include likes_count so the counter reacts instantly
    setPosts(prevPosts => prevPosts.map(p => {
