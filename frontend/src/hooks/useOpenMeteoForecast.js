...
// v3.9.3: Module-level rate limiter G현 shared across all instances
...
export const useOpenMeteoForecast = ({ latitude, longitude, activeModel = 'GFS', enabled = true }) => {
  ...
  const fetchForecast = useCallback(async () => {
    ...
    // v3.9: Coarser dedup grid (1-� instead of 0.1-�) G현 prevents 30+ fetches while panning
    const fetchKey = `${latitude.toFixed(0)}_${longitude.toFixed(0)}_${activeModel}`;
    if (fetchKey === lastFetchKey.current) return;

    // v3.9: Global rate limit G현 min 30s between any forecast fetch
    const now = Date.now();
    if (now - lastGlobalFetchTime < MIN_FETCH_INTERVAL) {
      return; // Silently skip G현 grid engine needs the rate budget
    }
  ...
  // v3.9.3: 15s debounce G현 gives wind grid engine a head start on the rate limit
  useEffect(() => {
