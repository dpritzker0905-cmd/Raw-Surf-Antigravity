...
import { Camera, Check, ArrowLeft, CreditCard, Coins, Loader2, RefreshCw } from 'lucide-react';
...
const JumpInFlow = ({ photographer, onBack, onSuccess }) => {
  ...
  const [step, setStep] = useState(isCameraAuthorized ? 'selfie' : 'preflight');
  ...
  // Auto-start camera when on selfie step
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ 
          top: '60px',
          bottom: step === 'payment' || step === 'selfie' ? '80px' : '0',
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center">
              ...
              <div className="mt-4 p-4 bg-cyan-500/20 border-2 border-cyan-500/40 rounded-xl w-full md:max-w-[400px]">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-cyan-500/30 flex items-center justify-center shrink-0">
                    <span className="text-2xl">{'\u{1F3C4}'}</span>
                  </div>
                  <div>
              ...
              <div className="hidden md:flex gap-3 justify-center md:max-w-[400px] mx-auto w-full mt-4">
                {!selfieUrl ? (
                  <Button aria-label="Capture selfie"
                  ...
                  <>
                    <Button aria-label="Retake selfie"
      ...
      {/* Fixed Bottom - Selfie Controls (Mobile) */}
      {step === 'selfie' && (
...
export default React.memo(JumpInFlow);
