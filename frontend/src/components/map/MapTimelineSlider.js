...
export const MapTimelineSlider = ({
  ...
  if (radarMode && radarFrames.length > 0) {
    ...
    return (
      <div className={`absolute bottom-[64px] md:bottom-0 left-0 right-0 z-[1000] backdrop-blur-xl border-t ${isLight ? 'border-gray-200' : 'border-zinc-800'} ${sliderBg}`} style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
