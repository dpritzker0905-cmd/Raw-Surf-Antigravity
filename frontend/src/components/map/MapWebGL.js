...
const MapWebGL = ({
  ...
  return (
    ...
    >
      ...
      {/* --- WEATHER LAYERS PLACEHOLDER --- */}
      {/* 
        Phase 1: Backend Data & GFS Baseline.
        These layers will render CustomLayerInterface with WebGL particle engines 
        (e.g., @geoql/maplibre-gl-wind) using textures generated by the Python worker.
      */}
      {activeLayers.includes('wind') && (
        <Source id="wind-layer-placeholder" type="geojson" data={{ type: 'FeatureCollection', features: [] }}>
          <Layer id="wind-particles" type="circle" paint={{'circle-color': '#2dd4bf', 'circle-opacity': 0.3}} />
        </Source>
      )}
      {activeLayers.includes('swell') && (
        <Source id="swell-layer-placeholder" type="geojson" data={{ type: 'FeatureCollection', features: [] }}>
          <Layer id="swell-heatmap" type="heatmap" paint={{'heatmap-color': ['interpolate', ['linear'], ['heatmap-density'], 0, 'rgba(33,102,172,0)', 1, 'rgb(103,169,207)']}} />
        </Source>
      )}
      {/* ---------------------------------- */}
