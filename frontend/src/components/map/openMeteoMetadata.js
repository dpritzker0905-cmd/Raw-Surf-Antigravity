...
export async function fetchModelMetadata(modelToCheck, MODEL_METADATA_CACHE, onMetadataChanged) {
    ...
    MODEL_METADATA_PROMISES[modelToCheck] = fetch(`https://openmeteo.s3.amazonaws.com/data_spatial/${modelToCheck}/latest.json?skip_intercept=true`)
      ...
      .then(data => {
        ...
        const result = {
          ...
          // Bootstrap axes and derived UI aliases are not provider observations.
          // Keep the completed provider manifest intact for the decoder.
          sourceMetadata: data,
        };
  ...
  // Warm live callers already bypass this function. Cold callers must await the
  // shared provider request before choosing a run/time axis for the decoder.
