...
const AppLayout = ({ children, hideNav = false, hideTopNav = false }) => {
 ...
 {/* Violation awareness banner -- appears below nav when user has strikes */}
      ...
      <main
        id="main-content"
        className={`${mainBgClass} ${showSidebar ? 'md:ml-16 xl:ml-[200px]' : ''} ${showSidebar && !hideRightSidebar ? 'lg:mr-[200px]' : ''} ${showTopNav ? 'pt-14 md:pt-0' : ''} ${showBottomNav ? 'pb-20 md:pb-0' : ''} transition-colors duration-300 ${impersonationPadding} show-scrollbar`}
