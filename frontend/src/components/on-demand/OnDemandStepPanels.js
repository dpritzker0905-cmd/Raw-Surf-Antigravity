/**
 * OnDemandStepPanels.js G현 Extracted from OnDemandRequestDrawer.js
        ...
        {step === 'crew' && (
          <div className="p-4 sm:p-6 space-y-5">
            ...
            <div className={`relative p-4 sm:p-6 rounded-2xl overflow-visible ${isLight ? 'bg-gradient-to-b from-cyan-100 via-blue-50 to-white' : 'bg-gradient-to-b from-cyan-900/30 via-blue-900/20 to-zinc-900'}`}>
              {/* Wave pattern background G현 pointer-events-none so quick-add pills remain clickable */}
                  ...
                  return (
                    <div className="mt-4 space-y-2">
                      ...
                      {suggestions.length > 0 ? (
                        ...
                        <p className={`text-xs ${textSecondary}`}>No recent connections G현 use the search to find crew members.</p>
                      )}
                    </div>
                  );
                })()
                ...
                <div className="mt-4 relative" style={{ zIndex: 50 }}>
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      ...
                      />
                      {/* Autocomplete Dropdown G현 API results OR inline following list */}
            ...
            <div className={`p-4 rounded-xl ${isLight ? 'bg-gradient-to-br from-cyan-50 to-blue-50' : 'bg-gradient-to-br from-cyan-500/10 to-blue-500/10'} border border-cyan-400/30`}>
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className={textSecondary}>Your Share (Captain) G현 {totalPrice > 0 ? ((captainPayAmount / totalPrice) * 100).toFixed(0) : 0}%</span>
                  <span className="font-medium text-yellow-400">${captainPayAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className={textSecondary}>Crew Covers G현 {totalPrice > 0 ? ((crewCoversAmount / totalPrice) * 100).toFixed(0) : 0}%</span>
