/**
 * GifPicker.js G현 Tenor API-powered GIF picker for MessagesPage.
 *
...
const GifPicker = ({ show, onSelect, onClose }) => {
  ...
  }, [show]); // intentionally omits fetchTrending from deps G현 only run once on first open
  ...
  // Debounced search G현 400ms delay with 2-char minimum
  useEffect(() => {
    const id = setTimeout(() => {
  ...
  const fetchTrending = async (loadMore = false) => {
    ...
    try {
      ...
      // Silent G현 picker just shows empty state
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      {/* GIF Grid G현 2-column masonry with infinite scroll */}
      <div
        ref={gridRef}
