/**
 * ProfileHeaderSection - Avatar, username, stats row, badges, bio, social links
 * Extracted from Profile.js for modularization (v70)
 */
import React from 'react';
import { Camera, MapPin, Check, Flame, Heart, Radio, Instagram, Globe, ExternalLink, Loader2, Trophy } from 'lucide-react';
...
var ProfileHeaderSection = ({
  ...
  isPhotographer, isProfilePhotographer,
  ...
  <div className="flex flex-col items-center mb-6 relative">
    ...
    {isOwnProfile && !notesHook.userNote && (
      <button
        onClick={() => { notesHook.setNoteText(''); notesHook.setShowNoteModal(true); }}
        className="mb-2"
    ...
    <div className="flex items-center justify-center gap-8 mb-4">
      ...
      <button onClick={onFollowersClick} className="text-center cursor-pointer hover:opacity-70 transition-opacity" data-testid="followers-count-btn">
      ...
      <div className="w-px h-8 bg-border" />
      <button onClick={onFollowingClick} className="text-center cursor-pointer hover:opacity-70 transition-opacity" data-testid="following-count-btn">
    ...
    <div className="flex items-center justify-center gap-2 flex-wrap mb-3">
      ...
      {profile.subscription_tier && profile.subscription_tier !== 'free' && (
        <Badge variant="outline" className="border-emerald-400 text-emerald-400 text-xs">{profile.subscription_tier}</Badge>
      )}
      {streak.current_streak > 0 && (
        <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30 text-xs">
          <Flame className="w-3 h-3 mr-1" />{streak.current_streak} day streak
        </Badge>
      )}
      {impactScore?.is_photographer && impactScore?.impact_score?.total_credits_given > 0 && (
        <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/30 text-xs">
          <Heart className="w-3 h-3 mr-1" />{impactScore.impact_score.level?.emoji} {impactScore.impact_score.total_credits_given.toFixed(0)} given
        </Badge>
      )}
      {gamificationStats.total_xp > 0 && <XPDisplay xp={gamificationStats.total_xp} size="sm" />}
      {profile.elite_tier === 'grom_rising' && (
        <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-xs" data-testid="grom-competes-badge">
          <Trophy className="w-3 h-3 mr-1" />Competes
        </Badge>
      )}
    </div>
    ...
    {isProfilePhotographer && profileUserId && (
      <div className="w-full max-w-sm mb-3"><TrustSignalBadges profileId={profileUserId} compact /></div>
    )}
    ...
    {/* Bio */}
    {profile.bio && (
      <p className="text-sm text-muted-foreground text-center max-w-sm mb-3" data-testid="profile-bio">{profile.bio}</p>
    )}
