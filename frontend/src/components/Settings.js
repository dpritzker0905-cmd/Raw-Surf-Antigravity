...
const SurfModeCard = ({ textPrimaryClass, textSecondaryClass, cardBgClass }) => {
  ...
  const [verificationStatus, setVerificationStatus] = useState(null); // null, 'pending', 'approved'
  ...
  const fetchVerificationStatus = async () => {
    try {
      ...
      if (proReq) setVerificationStatus(proReq.status); // e.g. 'pending', 'approved', 'rejected'
    } catch (e) {
      // Non-blocking
    } finally {
  ...
  const handleModeSelect = async (mode) => {
    ...
    try {
      ...
      setSurfMode(user?.surf_mode || 'casual'); // revert
    } finally {
      setSaving(false);
    }
  };
    ...
    <Card className={`${cardBgClass} mb-4 transition-colors duration-300`}>
      <CardHeader>
        <CardTitle className={`${textPrimaryClass} flex items-center gap-2`}>
          ...
          Surf Mode
          {isLegend && (
            <span className="ml-auto px-2 py-0.5 bg-amber-500/20 text-amber-400 text-xs rounded-full border border-amber-500/30 flex items-center gap-1">
              🎖️ Legend
            </span>
          )}
          {isVerifiedPro && !isLegend && (
            <span className="ml-auto px-2 py-0.5 bg-emerald-500/20 text-emerald-400 text-xs rounded-full border border-emerald-500/30 flex items-center gap-1">
              ✅ Verified Pro
            </span>
          )}
        </CardTitle>
          ...
          <div className="space-y-3">
            {loadingVerif ? (
              ...
              <div className="space-y-3">
                ...
                <WslVerificationForm wslForm={wslForm} setWslForm={setWslForm} onSubmit={handleWslSubmit} submitting={submittingWsl} textSecondaryClass={textSecondaryClass} />
              </div>
            ) : (
              <WslVerificationForm wslForm={wslForm} setWslForm={setWslForm} onSubmit={handleWslSubmit} submitting={submittingWsl} textSecondaryClass={textSecondaryClass} />
            )}
  ...
  return (
    <div className={`pb-20 ${mainBgClass} min-h-screen transition-colors duration-300`} data-testid="settings-page">
      <div className="max-w-md mx-auto p-4">
        ...
        {/* Friends Section */}
        <Card className={`${cardBgClass} mb-4 transition-colors duration-300`} data-testid="friends-settings-card">
          <CardHeader className="cursor-pointer" onClick={() => toggleSection('friends')}>
            <div className="flex items-center justify-between">
              <CardTitle className={`${textPrimaryClass} flex items-center gap-2`}>
              ...
              <ChevronDown className={`w-5 h-5 ${textSecondaryClass} transition-transform ${expandedSections.friends ? 'rotate-180' : ''}`} />
            </div>
          </CardHeader>
            ...
            {friendsLoading ? (
              ...
              <>
                ...
                {pendingRequests.length > 0 && (
                  <div className="space-y-2">
                    <h4 className={`text-sm font-medium ${textSecondaryClass}`}>
                      Pending Requests ({pendingRequests.length})
                    </h4>
                    {pendingRequests.map(request => (
                      <div key={request.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center overflow-hidden">
                          {request.requester.avatar_url ? (
                            ...
                            <span className="text-black font-bold">{request.requester.full_name?.charAt(0)}</span>
                          )}
                        </div>
                        <div className="flex-1">
                          <p className={textPrimaryClass}>{request.requester.full_name}</p>
                          <p className={`text-xs ${textSecondaryClass}`}>{request.requester.role}</p>
                        </div>
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                ...
                {friends.length > 0 ? (
                  <div className="space-y-2">
                    <h4 className={`text-sm font-medium ${textSecondaryClass}`}>Your Friends</h4>
                    {friends.map(friend => (
                      <div key={friend.friendship_id} className="flex items-center gap-3 p-3 rounded-lg bg-muted">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center overflow-hidden">
                          {friend.user.avatar_url ? (
                            <img src={friend.user.avatar_url} alt="" className="w-full h-full object-cover" />
                          ) : (
                            <span className="text-black font-bold">{friend.user.full_name?.charAt(0)}</span>
                          )}
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <p className={textPrimaryClass}>{friend.user.full_name}</p>
                            {friend.user.is_shooting && (
                              <span className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 text-[10px] rounded">
                                LIVE
                              </span>
                            )}
                          </div>
                          <p className={`text-xs ${textSecondaryClass}`}>{friend.user.role}</p>
                        </div>
                        <Button
                          size="sm"
                          variant="ghost"
                  ...
                  <p className={`text-center py-4 ${textSecondaryClass}`}>
                    No friends yet. Connect with surfers and photographers!
                  </p>
                ) : null}
              </>
            )}
