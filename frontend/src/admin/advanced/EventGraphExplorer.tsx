...
export const EventGraphExplorer: React.FC = () => {
  ...
  const getEventIcon = (domain: string) => {
    switch (domain) {
      ...
      case 'booking': return <ShieldCheck className="w-4 h-4 text-red-400" />;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4 lg:col-span-1 border-r border-slate-850 pr-4">
          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Search Trace Token</label>
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-950 border border-slate-900 text-xs px-8 py-2 rounded-lg text-slate-200 focus:outline-none focus:border-cyan-500 w-full"
              />
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Domain Category Filter</label>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-all flex justify-between items-center text-xs ${
                        isSelected
                          ? 'bg-cyan-500/10 border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.05)]'
                          : 'bg-slate-950/40 border-slate-900 hover:border-slate-800 text-slate-400'
                      }`}
                    >
                      <div className="space-y-1 truncate max-w-[85%]">
                        <span className={`font-mono text-[10px] font-bold ${isSelected ? 'text-cyan-400' : 'text-slate-300'}`}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-[9px] font-mono bg-slate-900 px-1.5 py-0.5 rounded border border-slate-850 text-slate-400 font-bold">
                          {chain.length}
                        </span>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-slate-950 border border-slate-900 rounded-xl p-4 flex flex-col justify-between min-h-[460px] relative overflow-hidden">
            ...
            {selectedCorrelationId ? (
              <div className="space-y-4 z-10 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center border-b border-slate-900 pb-3">
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block">Active Causal Chain</span>
                      <h3 className="text-xs font-mono font-bold text-cyan-400 flex items-center gap-1.5">
                        <Link className="w-3.5 h-3.5" />
                        {selectedCorrelationId}
                      </h3>
                    </div>
                    <span className="text-[9px] font-mono bg-cyan-500/10 text-cyan-400 px-2 py-0.5 rounded border border-cyan-500/20 font-bold">
                  ...
                  <div className="flex justify-between w-full px-8 relative z-10">
                    {selectedChain.map((evt, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`w-9 h-9 rounded-full bg-slate-950 border flex items-center justify-center transition-all duration-300 group-hover:scale-110 z-10 ${getEventGlow(domain)}`}>
                            {getEventIcon(domain)}
                          </div>
                          
                          <span className="text-[9px] font-mono font-bold text-slate-300 mt-2 text-center truncate max-w-full px-1 uppercase tracking-tight block">
                            {evt.event_type.replace(/_/g, ' ').substring(0, 16)}
                          </span>

                          <span className="text-[8px] font-mono text-slate-500 mt-0.5 block">
                            {new Date(evt.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                          </span>

                          {/* Hover Tooltip card details */}
                          <div className="absolute top-12 scale-0 group-hover:scale-100 transition-all origin-top bg-slate-950/95 border border-slate-800 text-[9px] p-3 rounded-lg w-48 shadow-2xl z-30 font-mono space-y-1 text-left pointer-events-none">
                            <div className="text-cyan-400 font-bold border-b border-slate-900 pb-1 mb-1 uppercase text-[8px]">
                              {evt.event_type.replace(/_/g, ' ')}
                            </div>
                            <div><span className="text-slate-500">ID:</span> <span className="text-slate-300">{evt.event_id.substring(0, 8)}...</span></div>
                            <div><span className="text-slate-500">Time:</span> <span className="text-slate-300">{new Date(evt.timestamp).toLocaleTimeString()}</span></div>
                            <div><span className="text-slate-500">MCP Source:</span> <span className="text-slate-300">{evt.source_mcp}</span></div>
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="space-y-2">
                  <h4 className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Chronological Node Ledger</h4>
                  <div className="space-y-1.5 max-h-[140px] overflow-y-auto scrollbar-thin pr-1">
                    {selectedChain.map((evt, idx) => {
                      ...
                      return (
                        <div 
                          key={evt.event_id} 
                          className="bg-slate-900/30 border border-slate-900/60 rounded px-3 py-2 flex items-center justify-between text-[10px] font-mono"
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-slate-500">#{idx + 1}</span>
                            {getEventIcon(domain)}
                            <span className="text-slate-300 font-bold uppercase">{evt.event_type.replace(/_/g, ' ')}</span>
                          </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 z-10">
                ...
                <h4 className="text-slate-400 font-semibold text-xs mb-1">Causal Graph Viewer Active</h4>
