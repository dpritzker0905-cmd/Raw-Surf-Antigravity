...
| 7 | Estimates ABSENT during each ~1–1.5h ingest window (every 4h) — old products gone before new land | ROOT PROVEN (run 29052535445 log): EURO native prune 22:41 deletes old est tail (68/layer ×4, silent — L2-lazy files skip the per-file log) → extend job 23:01 (2s compute) → L2 upload backlog drains 23:24 → serving 23:30. Gap ≈ 35-45min/cycle. In-job GFS-ext = 0 in production (see #15) | FIXED `cf0b4b23` (estimated products superseded only by a NEWER ESTIMATED generation — mirror of the 07-06 auth guard; cycle-end sweep deletes old set AFTER new exists; kill `INGEST_PRUNE_PRESERVE_ESTIMATES=0`). ⚠️ VERIFY next cycles: EURO prune count drops ~68→~34/layer; probe shows no 404 window |
