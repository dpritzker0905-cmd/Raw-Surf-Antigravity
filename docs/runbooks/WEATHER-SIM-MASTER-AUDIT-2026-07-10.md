...
| 22 | Cold dynamic global ICON wind builds >40s (both 3h-aligned and unaligned hours 504 at the fitted ceiling; success depends on background completion + caching) — far-hour ICON wind cold-scrubs feel bad and grind the 1-CPU box | live curls 07-10 eve (+168/+170h: 504 @40s ×2; +204h band served only after a prior bg completion cached it) | OPEN — durable fix = PRE-BAKE ICON wind 3-hourly to 336h in the ingest cycle via `extend_icon_wind_to_14d` on the GH runner (north-star pre-bake pattern, mirrors EURO marine estimates). Interim: hold-last-frame + safe-zero fallbacks already smooth the client |
