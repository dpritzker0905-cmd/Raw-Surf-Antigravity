...
While building the auth-gate scanner, extended it to flag any route accepting a bare `user_id` parameter (query/path/body) with **no** auth dependency at all (`get_current_admin`, `get_current_user`, or `get_optional_user_id`) anywhere in its signature. Initial result: 309 of 930 routes. **This count was itself corrected in a dedicated follow-up deep dive** — the same substring-matching gap this section exists to catch (missing the `get_user_id_from_jwt_or_query` migration-bridge helper) had contaminated this count too. **Corrected figure: 221 of 930 routes (24%).** Confirmed there is **no global authentication middleware** (`server.py` only registers `GZipMiddleware` and `CORSMiddleware`) — so for these 221 routes, whatever `user_id` the caller supplies in the request is trusted as-is, with no cryptographic verification it matches an authenticated session.

**This is explicitly out of scope for this pass and was not touched.** It is not an admin-panel issue — it's a foundational, app-wide identity-verification architecture question (OWASP API1:2023 Broken Object Level Authorization). A full dedicated forensic deep dive — git history (this exact issue was declared "fixed" twice before, `c1526cdc` and `84236b0c`), frontend token-flow confirmation, severity categorization, and a phased remediation plan — is written up in **`docs/runbooks/HANDOFF-2026-07-12-USERID-AUTH-ARCHITECTURE-BOLA-REVIEW.md`**. Not folded into this runbook further; that document is now the canonical source for this finding.
