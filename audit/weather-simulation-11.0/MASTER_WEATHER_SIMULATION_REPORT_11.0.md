...
| **REPAIR — the GL test lane is a SKIP, not a PASS** | The repo's own spec records *"engine resident + diag renderable + **ZERO field pixels** on this headless runner (a white void; SwiftShader-class silent no-paint)"*. Fix: `channel:'chromium'` + GPU launch args. **A green CI run in that lane currently proves nothing.** |
