...
| V2 | `gh run download 31751873373 --name playwright-report` — the WebKit weather-test failure video + trace | Discriminates *timeout budget* from *real WebKit defect*. Decide **before** looking: page rendered at the deadline ⇒ per-project timeout; still spinning ⇒ open a defect | minutes | **expires 2026-08-27** |
